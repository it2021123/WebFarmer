...
const onFormSubmit = async () => {
  ...
  const { data, error, loading } = useRemoteDataPUT(`http://localhost:3030/api/Admin/farmers/update-farmer/${userId}`, true, requestData);

  if (data) {
    console.log('User updated successfully:', data);

    await router.push({name: 'farmer', params: {farmerId: userId}});
  } else {
